...
-(void)setUp{
    ...
    self.placeholderView.backgroundColor = RGBA(44, 48, 51, 1);
    [self addSubview:self.placeholderView];
    [self.placeholderView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(self).insets(UIEdgeInsetsMake(0, 0, 0, 0));
    }];
    
    UIImageView *placeholderImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"icon_placeholder"]];
    ...
    [placeholderImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerY.equalTo(self.placeholderView.mas_centerY);
        make.width.height.equalTo(self.placeholderView.mas_width).multipliedBy(0.3);
    }];
}
